...
const myDays = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
    ...
    const swahiliDay = ['Jumapili', 'Jumatatu', 'Jumanne', 'Jumatano', 'Alhamisi', 'Ijumaa', 'Jumamosi'][d.getDay()];

    return `${thisDay}, ${day} - ${myMonths[bulan]} - ${year}`;
}

exports.jam = (numer, options = {}) => {
    let format = options.format ? options.format : "HH:mm:ss"
    const timeZone = options.timeZone || 'Africa/Nairobi'
    let jam = moment(numer).tz(timeZone).format(format)
