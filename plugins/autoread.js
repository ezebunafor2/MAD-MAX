...
async function autoreadCommand(sock, chatId, message) {
    try {
       ...
       // Show status if no arguments, otherwise toggle based on argument
if (args.length === 0) {
    // Show current status instead of toggling
    await sock.sendMessage(chatId, {
        text: `🔍 *Auto-Read Status:* ${config.enabled ? '✅ ON' : '❌ OFF'}\n\n` +
              `Usage:\n` +
              `• ${global.PREFIX || '.'}autoread on - Enable auto-read\n` +
              `• ${global.PREFIX || '.'}autoread off - Disable auto-read`,
        contextInfo: {
            forwardingScore: 1,
            isForwarded: true,
            forwardedNewsletterMessageInfo: {
...
function isBotMentionedInMessage(message, botNumber) {
    ...
    if (textContent) {
        ...
        const botNames = [global.botname?.toLowerCase(), 'bot', '404', 'MAD-MAX'];
